...
def get_responsible_party(citedResponsiblePartys, agent_config):
    for party in citedResponsiblePartys:
        if party["role"] == agent_config.get('role'):
            ...
            if publisher_name:
                code = agent_code or agent_config.get('code')
        ...
        for key in keywords:
            if thesaurus_id and (thesaurus_id in key['thesaurus-identifier'] or thesaurus_id in key['thesaurus-title']):
                for k in key['keyword']:
                    query = Session.query(DCATAPITTagVocabulary).filter(DCATAPITTagVocabulary.text==k, DCATAPITTagVocabulary.tag_name.in_(tag_names_list))
...
def get_agent(agent_string, agent_config):
    ## Agent Code
    code_regex = agent_config.get('code_regex')
    agent_code = re.search(code_regex.get('regex'), agent_string)

    if agent_code:
        regex_groups = code_regex.get('groups')
    ...
    ## Agent Name
    name_regex = agent_config.get('name_regex')
    agent_name = re.search(name_regex.get('regex'), agent_string)

    if agent_name:
        regex_groups = name_regex.get('groups')
